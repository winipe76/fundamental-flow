...
function signalScore(status: unknown, value: unknown, numericValues: number[]) {
  ...
  return finite(value) ? percentile(numericValues, value) : null;
}
...
export function rankSnapshots(rows: RankingSnapshot[]) {
  ...
  const qoqValues = eligible.filter((row) => row.eps_qoq_status === "growth" && finite(row.eps_qoq_pct)).map((row) => row.eps_qoq_pct as number);
  ...
  return eligible.map((row) => {
    const epsYoy = signalScore(row.eps_yoy_status, row.eps_yoy_pct, epsValues);
