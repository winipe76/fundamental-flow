...
test("uses the five-company validation universe",()=>{for(const ticker of ["MU","TER","ALNY","PLTR","NVDA"])assert.match(page,new RegExp(`ticker:\"${ticker}\"`));assert.match(page,/Validation Universe/);assert.match(page,/3-MONTH VALIDATION UNIVERSE/);assert.doesNotMatch(page,/ticker:\"MSFT\"/)});
