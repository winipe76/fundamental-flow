...
test("uses consistent actual growth and NTM definitions",()=>{for(const label of ["최근 분기 EPS","최근 분기 Revenue","NTM EPS","1개월 수정률","3개월 수정률"])assert.match(page,new RegExp(label))});
test("supports requested screener filters and sorting",()=>{for(const label of ["EPS YoY 높은 순","Revenue YoY 높은 순","흑자 전환 기업","NTM EPS 상향 기업"])assert.match(page,new RegExp(label))});
