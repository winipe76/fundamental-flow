  ...
  for (const ticker of TEST_TICKERS) {
    try {
      ...
      await runtime.DB.prepare(`INSERT INTO fundamental_snapshots (
        ...
        ntm_eps_change_1m_pct,ntm_eps_change_3m_pct
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(ticker,snapshot_date) DO UPDATE SET
        ...
        revenue_yoy_pct=excluded.revenue_yoy_pct,ntm_eps=excluded.ntm_eps,ntm_components=excluded.ntm_components,
        ntm_eps_change_1m_pct=excluded.ntm_eps_change_1m_pct,ntm_eps_change_3m_pct=excluded.ntm_eps_change_3m_pct`
      ).bind(
        ticker,snapshotDate,null,value.epsDefinition,null,null,value.actualTrailingRevenue,value.operatingIncome,value.operatingMargin,
        value.freeCashFlow,value.fcfMargin,null,null,null,null,JSON.stringify(value.missingFields),value.collectionStatus,collectedAt,
        ...
        value.revenueYoyPct,value.ntmEps,JSON.stringify(value.ntmComponents),ntmEpsChange1mPct,ntmEpsChange3mPct
      ).run();
      results.push({ ...value, ntmEpsChange1mPct, ntmEpsChange3mPct, snapshotDate });
    } catch (error) {
