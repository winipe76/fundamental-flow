...
test("keeps the Nasdaq 100 pilot ranking separate from Fundamental Overview",()=>{for(const label of ["NASDAQ 100 · PILOT RANKING","TOP 10","Fundamental Score","Nasdaq 100 업데이트","Dashboard 선정 상태에는 반영하지 않습니다"])assert.match(page,new RegExp(label))});
...
test("uses the five-company validation universe",()=>{for(const ticker of ["MU","TER","ALNY","PLTR","NVDA"])assert.match(page,new RegExp(`ticker:\"${ticker}\"`));assert.match(page,/Validation Universe/);assert.match(page,/OBSERVED COMPANIES/);assert.doesNotMatch(page,/ticker:\"MSFT\"/)});
